...
class Detail extends React.Component {
		...
		ajax.get('https://api.github.com/repos/facebook/react/commits')
			.end((error, response) => {
				if (!error && response) {
					...
					console.log('Error fetching commits from github', error);
				}
			}
		);
		...
		ajax.get('https://api.github.com/repos/facebook/react/forks')
			.end((error, response) => {
				if (!error && response) {
					...
					console.log('Error fetching forks from github', error);
				}
			}
		);
		...
		ajax.get('https://api.github.com/repos/facebook/react/pulls')
			.end((error, response) => {
				if (!error && response) {
					...
					console.log('Error fetching pulls from github', error);
				}
			}
		);
	}
	...
	render() {
		...
		return (<div> 	
			...
			<button onClick={ this.showForks.bind(this) }>Forks</button>
			<button onClick={ this.showPulls.bind(this) }>Pulls</button>
			{content}
		</div>);
	}
}
